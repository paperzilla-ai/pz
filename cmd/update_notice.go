...
func buildUpdateNotice(current string, release update.Release, useColor bool) string {
	...
	if update.IsSourceVersion(current) {
		...
		return fmt.Sprintf("%s this is a source build, not an official tagged release. Latest release: %s. Run `pz update` for upgrade instructions.", label, terminalSafeInline(displayVersion(release.TagName)))
	}
	...
	return fmt.Sprintf("%s pz %s is out of date. Latest release: %s. Run `pz update` to upgrade and get the latest fixes.", label, terminalSafeInline(displayVersion(current)), terminalSafeInline(displayVersion(release.TagName)))
}
